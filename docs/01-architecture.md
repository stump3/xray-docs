...
| XHTTP + Reality + TLS (nested) | ❌ нет | Очень низкое | Высокая | ❌ нет | Высокая |
  ...
  xver: 1  ← PROXY protocol → Nginx получает реальный IP
    ...
    └── Fallback → :2023 (VLESS+XHTTP plain)
                     ...
                     └── Nginx :cdn_domain (grpc_pass → :2023)  (H)
                              └── REALITY nested: client via Reality → XHTTP → Nginx (K)
...
| K | VLESS+XHTTP+REALITY (nested) | Клиент подключается как M (REALITY), внутри XHTTP. Конфиг общий с H |
...
| ✅ Плюсы | ❌ Минусы |
|---|---|
| Reality без зависимости от стороннего сайта — декой на своём IP | Complex chain: Xray→Nginx→Xray (XHTTP nested) |
